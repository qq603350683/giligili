...
func WechantLogin(code string) *WechatLoginResult {
	...
	if wechat_user == nil {
		...
		user_plan.DetailJson = `{"w":129,"h":120,"texture":"hero/hero2.png","bullets":[{"id":1,"title":"A导弹","w":20,"h":20,"p":3,"a":0,"level":1,"max_level":3,"rate":100,"max_rate":90,"speed":20,"max_speed":25,"texture":"bullet/10.png"}],"skills":[{"id":1,"title":"光速射线","w":50,"h":50,"p":1,"a":0,"level":2,"max_level":4,"rate":180,"max_rate":160,"speed":50,"max_speed":60,"height":9999999,"texture":"bullet/skill1.png"}]}`
		...
		//b := user.ChangePlan(user_plan.UpID)
		//if b == false {
		//	db.Rollback()
		//	return wechat_login_result
		//}
		...
		err = db.Create(new_wechat_user).Error
		if err != nil {
			db.Rollback()
			log.Println(err.Error())
			return wechat_login_result
