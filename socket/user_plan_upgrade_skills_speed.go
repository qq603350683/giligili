...
func UserPlanUpgradeSkillsSpeed(params Params) {
	up_id := 0
	speed := 0
	...
	for i, b := range(user_plan.Detail.Skills) {
		if speed == 0 {
			speed = b.Speed

			if b.Speed >= b.MaxSpeed {
				SendMessage(model.UserInfo.UID, serializer.JsonByte(http.StatusInternalServerError, "速度已达最大等级", nil, ""))
				return
			}
		}
		...
		user_plan.Detail.Skills[i] = b
	}

	// 消耗的金币
	consume_gold := speed * 100
