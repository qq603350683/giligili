...
func UserPlanUpgradeBulletRate(params Params) {
	id := 0
	up_id := 0
	index := -1
	bullet := model.Bullet{}
	...
	// 已达到最高强化级别
	if user_plan.Detail.Bullets[index].MaxRate <= user_plan.Detail.Bullets[index].Rate {
	...
	// 消耗的钻石
	consume_diamond := bullet.Rate * 3

	if model.UserInfo.Diamond < consume_diamond {
		SendMessage(model.UserInfo.UID, serializer.JsonByte(http.StatusInternalServerError, fmt.Sprintf("钻石不足 %d", consume_diamond), nil, "金币不足"))
		return
	}
